...
敦煌壁画承载千年丝路艺术文化，飞天、瑞兽、菩萨等形象极具东方美学价值，但传统敦煌壁画创作门槛高、绘制流程繁琐，普通大众难以亲身参与其中。

**身绘** 是一款基于手势交互生成敦煌壁画的创意工具。项目结合 **MediaPipe** 人体、手部、人脸关键点检测技术，打造实时姿态驱动的敦煌壁画交互系统。用户只需面对摄像头做出指定的肢体与手部姿势，程序即可自动识别并匹配渲染对应的敦煌风格壁画形象；同时支持通过手势操控添加飞天拖尾飘带、壁画纹路等装饰元素。以身体为画笔，以手势为语言，零美术基础也能在数字画布上创作独一无二的敦煌风格艺术作品。
...
```bash
# 克隆仓库
git clone https://github.com/your-username/shenhui.git
cd shenhui
...
本项目仅用于学习与艺术创作目的。敦煌壁画素材版权归原作者所有。
